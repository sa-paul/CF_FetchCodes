...
newElement.className = "program";
let newContent = document.createElement("p");
newElement.appendChild(newContent);
newContent.className = "programContent";
let TextInsideNode = document.createTextNode("This is inserted by js.");
newContent.appendChild(TextInsideNode);

let element = document.getElementById(`sidebar`);
element.appendChild(newElement);


// another method to add/inject html by js
...
async function getSubmissionId(contestIds, problemIds, handles) {
  let apiUrl = `https://codeforces.com/api/contest.status?contestId=${contestIds}&from=1&count=10&handle=${handles}`;
...
// getSubmissionId(contestId, problemId, handle)
//   .then(submissionId => {
//     console.log(submissionId);
//   })
//   .catch(error => {
...
// setTimeout(() => {
  ...
  //   .then(submissionId => {
  //     console.log(submissionId);
  //     // Fetching Code and Adding to Modal
  //     // fetchCodeFromSubmission(cntstid, submissionId)
  //     //   .then(function (code) {
  //     //     modalParaID.innerHTML = code;
  //     //   })
  //   })
